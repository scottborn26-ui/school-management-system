...
function AttendancePage() {
  ...
  const teacherStreamIds = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        streamIds: [...new Set((data ?? []).map((row) => row.stream_id))],
      };
    },
  });
  ...
  const records = useQuery({
    ...
    enabled: Boolean(streamId && slotId && attendanceDate),
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!streamId || !learningAreaId || !slotId) {
        throw new Error("Select a class, learning area and lesson first.");
      }
      const lesson = (lessonOptions.data ?? []).find((option) => option.id === slotId);
      if (!lesson) throw new Error("That lesson is not in your assigned timetable.");
      const allocation = (allocations.data ?? []).find(
        (item) => item.learning_area_id === lesson.learning_area_id,
      );
      if (!allocation) throw new Error("That lesson has no active teacher assignment.");
      const rows = (learners.data ?? []).map((learner) => ({
        ...
        timetable_slot_id: lesson.id,
        teacher_allocation_id: allocation.id,
      }));
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["attendance-records", schoolId, streamId, attendanceDate],
      });
    },
    onError: (error: Error) =>
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={() => save.mutate()}
              disabled={save.isPending || !streamId || !learningAreaId || !slotId || !hasUnsavedChanges}
