    ...
    queryFn: async () => {
      const [profile, settings, offerings, terms, events] = await Promise.all([
        ...
        supabase
          ...
          .select("id, title, event_type, start_date, end_date, notes, all_day, academic_year_id")
          .eq("school_id", schoolId)
          .order("start_date"),
      ]);
      return {
...
function CalendarTab({
  ...
  const eventMutation = useMutation({
    ...
    onSuccess: () => {
      setEventForm({ title: "", event_type: "other", start_date: "", end_date: "", notes: "" });
    ...
    <Card className="calendar-shell card-accent-attendance">
      ...
      <CardContent className="calendar-content settings-calendar-content space-y-5">
        ...
        <div className="calendar-events-panel border-t pt-5">
          ...
          <div className="grid gap-2 rounded-lg border p-3 md:grid-cols-[1.4fr_0.8fr_1fr_1fr_auto] md:items-end">
            <div className="space-y-1.5">
