...
function SchoolDashboardPage() {
  ...
  const isTeacherDashboard =
    ...
    !school.can("principal", "deputy", "super_admin", "admin", "registrar", "bursar");
    ...
    queryFn: async () => {
      ...
      const [
        ...
        isTeacherDashboard
          ...
          : supabase
              .from("payments")
              .select("amount, term_id, is_reversed, invoice_id, paid_at, learner_id")
              .eq("school_id", schoolId),
        isTeacherDashboard
          ? Promise.resolve({ data: [], error: null })
          : supabase
      ...
      if (previousPayments.error) throw previousPayments.error;
      if (feeItems.error) throw feeItems.error;
  ...
  const activeLearners = learners.filter((learner) => learner.status === "active");
  ...
  const previousTermPayments = (data?.previousPayments ?? [])
    .filter((payment) => !payment.is_reversed)
    .reduce((sum, payment) => sum + Number(payment.amount || 0), 0);
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-6 pb-8">
      <section className="relative overflow-hidden rounded-xl bg-sidebar px-5 py-5 text-sidebar-foreground shadow-lg shadow-primary/10 sm:px-8 sm:py-7">
        <div className="pointer-events-none absolute -right-12 -top-20 size-64 rounded-full border-[28px] border-sidebar-primary/15" />
        <div className="pointer-events-none absolute -bottom-24 right-36 size-52 rounded-full border-[20px] border-white/5" />
            ...
            {isTeacherDashboard ? (
              <>
                <Button
                  asChild
                  variant="secondary"
                  className="border border-white/20 bg-white/10 text-white shadow-xs backdrop-blur-xs transition-all duration-200 hover:border-white/40 hover:bg-white/20 hover:text-white hover:shadow-sm"
                >
                  <Link to="/attendance">
                ...
                <Button
                  asChild
                  className="bg-sidebar-primary text-sidebar-primary-foreground shadow-md shadow-sidebar-primary/20 transition-all duration-200 hover:scale-[1.02] hover:bg-sidebar-primary/90 hover:shadow-lg hover:shadow-sidebar-primary/30"
                >
                  <Link to="/marks">
                ...
                {upcomingEvents.length === 0 ? (
                  ...
                  upcomingEvents.map((event) => {
                    ...
                    return (
                      ...
                      >
                        <div className="grid size-10 shrink-0 place-items-center rounded-lg bg-amber-50 text-amber-700">
              ...
              <CardContent className="grid grid-cols-3 gap-2">
                {quickActions.map((action) => (
                  <Button
                    ...
                    asChild
                    variant="outline"
                    className="h-auto min-h-20 flex-col gap-1.5 rounded-xl border-border/70 px-1 py-2 text-[0.65rem] font-semibold leading-tight shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-all hover:-translate-y-0.5 hover:bg-teal-50 hover:text-teal-900 hover:shadow-sm focus-visible:ring-2 focus-visible:ring-primary/40"
                  >
                    <Link to={action.to}>
...
function ExamOfficerDashboard() {
  ...
  const examData = useQuery({
    ...
    queryFn: async () => {
      const [assessmentResult, learnerResult, markResult, areaResult] = await Promise.all([
        supabase
          .from("assessments")
  ...
  const activities = [
    ...
    [
      "Invigilators assigned for End Term 2",
      "by You",
    ...
    [
      "Results published for Mid Term Test",
      "by You",
      "26 May 2024, 04:20 PM",
      FileText,
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 rounded-xl border bg-card px-4 py-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-sm font-semibold">Tuesday, 28 May 2024</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        ...
        <Card className="rounded-xl border-border/70 shadow-sm">
          ...
          <CardContent className="space-y-3">
            {[
              [
                "3 exams are not yet assigned invigilators.",
                "Assign now",
                "border-red-300 bg-red-50 text-red-800",
                "/staff",
              ],
              [
                "Mark entry pending for candidates.",
                "Enter marks",
                "border-amber-300 bg-amber-50 text-amber-800",
                "/marks",
              ],
              [
                "Practical exams are due next week.",
                "View schedule",
                "border-blue-300 bg-blue-50 text-blue-800",
                "/exam-timetable",
              ],
            ].map(([message, action, alertClass, to]) => (
              <Link
                to={to}
